...
class ViT(torch.nn.Module):
    def __init__(
        ...
        self.patch_embedding = PatchEmbedding(
            img_size, patch_size, num_hiddens=embed_size
        )
        ...
        self.position_embedding = torch.nn.Parameter(
            torch.randn(1, num_steps, embed_size)
        )
        ...
        self.head = torch.nn.Sequential(
            torch.nn.LayerNorm(embed_size), torch.nn.Linear(embed_size, num_classes)
        )
    ...
    def forward(self, X):
        ...
        for block_num, block in enumerate(self.blocks):
            ...
            self.attention_weights[
                block_num
            ] = block.attention.attention.attention_weights
        return self.head(X[:, 0])  # gets only latent embedding



class ViTSequential(ViT):
    def __init__(self, img_size, patch_size, embed_size, mlp_num_hiddens, num_heads, num_blocks, emb_dropout, block_dropout, use_bias=False, num_classes=10) -> None:
        super().__init__(img_size, patch_size, embed_size, mlp_num_hiddens, num_heads, num_blocks, emb_dropout, block_dropout, use_bias, num_classes)
